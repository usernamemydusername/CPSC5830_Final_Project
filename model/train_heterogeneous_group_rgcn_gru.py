...
def apply_grouped_poi_features(
    data: HeteroData,
    data_dir: Path,
    mapping_path: Optional[Path] = None,
) -> tuple[HeteroData, List[str]]:
    """
    Replace data['poi'].x with grouped semantic POI features at training time.

    This does not modify hetero_graph.pt on disk and does not modify data['region'].x.
    It only reduces sparsity in POI node features.
    """
    feature_path = data_dir / "feature_names.json"
    if mapping_path is None:
        mapping_path = data_dir / "poi_group_mapping.json"
    ...
    poi_feature_names = feature_names["poi_feature_names"]
    groups = mapping.get("groups", mapping)

    x = data["poi"].x.float()

    if x.shape[1] != len(poi_feature_names):
        raise ValueError(
            f"POI feature dimension mismatch: data['poi'].x has {x.shape[1]} columns, "
            f"but feature_names.json lists {len(poi_feature_names)} POI features."
        )

    name_to_idx = {name: i for i, name in enumerate(poi_feature_names)}
    grouped_cols = []
    grouped_names = []
    used = set()

    # 1. Main semantic groups from poi_group_mapping.json
    for group_name, source_names in groups.items():
        idxs = [name_to_idx[name] for name in source_names if name in name_to_idx]
        if not idxs:
            continue

        # POI features are multi-hot, so max means "belongs to this group".
        grouped_cols.append(x[:, idxs].max(dim=1).values)
        grouped_names.append(group_name)
        used.update(idxs)

    # 2. Keep broad *_flag columns
    for i, name in enumerate(poi_feature_names):
        if name.endswith("_flag") and i not in used:
            grouped_cols.append(x[:, i])
            grouped_names.append(name)
            used.add(i)

    # 3. Put unmapped rare tags into tag-specific misc groups, not global other
    for prefix, misc_name in [
        ("amenity_", "amenity_misc"),
        ("shop_", "shop_misc"),
        ("tourism_", "tourism_misc"),
        ("public_transport_", "public_transport_misc"),
    ]:
        idxs = [
            i for i, name in enumerate(poi_feature_names)
            if name.startswith(prefix)
            and i not in used
            and not name.endswith("_flag")
        ]
        if idxs:
            grouped_cols.append(x[:, idxs].max(dim=1).values)
            grouped_names.append(misc_name)
            used.update(idxs)

    if not grouped_cols:
        raise ValueError(
            "POI grouping produced no features. Check poi_group_mapping.json and feature_names.json."
        )

    data["poi"].x = torch.stack(grouped_cols, dim=1).float()
    return data, grouped_names
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["raw", "grouped"],
        default="raw",
        help="raw keeps data['poi'].x unchanged; grouped maps sparse POI tags to semantic groups at training time."
    )
    parser.add_argument(
        "--poi-group-mapping",
        type=Path,
        default=None,
        help="Optional path to poi_group_mapping.json. Defaults to <data-dir>/poi_group_mapping.json."
    )
...
def main() -> None:
    ...
    if args.poi_feature_mode == "grouped":
        hetero_graph, grouped_poi_feature_names = apply_grouped_poi_features(
            hetero_graph,
            data_dir=args.data_dir,
            mapping_path=args.poi_group_mapping,
        )
    ...
    print(f"poi_feature_mode={args.poi_feature_mode}")
    if grouped_poi_feature_names is not None:
        print(f"grouped_poi_feature_names={grouped_poi_feature_names}")
    ...
    config.update({
        ...
        "poi_feature_mode": args.poi_feature_mode,
        "grouped_poi_feature_names": grouped_poi_feature_names,
        "poi_group_mapping": None if args.poi_group_mapping is None else str(args.poi_group_mapping),
    })
