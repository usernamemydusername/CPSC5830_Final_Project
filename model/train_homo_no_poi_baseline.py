...
def load_homogeneous_region_graph(
    data_dir: Path,
    gnn_type: str = "sage",
    region_feature_mode: str = "full",
) -> HomoGraph:
    hetero = torch.load(data_dir / "hetero_graph.pt", map_location="cpu", weights_only=False)
    x = hetero["region"].x.float()

    if region_feature_mode == "no_poi":
        # Keep only the 6 base region features:
        # centroid_x_z, centroid_y_z,
        # log1p_taxi_out, log1p_taxi_in,
        # log1p_taxi_total, log1p_destination_count.
        x = x[:, :6]
    elif region_feature_mode == "full":
        pass
    else:
        raise ValueError(f"Unknown region_feature_mode={region_feature_mode}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["full", "no_poi"],
        default="full",
        help="full uses all region features; no_poi keeps only the first 6 base region features and removes log1p_poi_count_* features.",
    )
...
def main() -> None:
    ...
    homo_graph = load_homogeneous_region_graph(args.data_dir, gnn_type=args.gnn_type, region_feature_mode=args.region_feature_mode).to(device)
    ...
    print(f"region_feature_mode={args.region_feature_mode}")
    ...
    config.update({
        ...
        "region_feature_mode": args.region_feature_mode,
    })
